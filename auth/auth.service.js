...
let AuthService = class AuthService {
    ...
    async Register(User) {
        ...
        if (users) {
            throw new common_1.UnauthorizedException({ mesaage: 'This number is using away....' });
        }
        else {
            this.usersService.AddUser(User);
        }
    }
    async generteToken(user) {
        const payload = {
            id: user.id,
            name: user.name,
    ...
    async ValidateUser(authDto) {
        const user = await this.usersService.getUserByNumber(authDto.number);
        if (user) {
